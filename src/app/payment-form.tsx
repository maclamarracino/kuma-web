...
// Remove the global declaration completely
// Instead, create a type for our use only
type MercadoPagoConstructor = new (publicKey: string) => MercadoPagoInstance
    ...
    const loadMercadoPago = async () => {
      try {
        ...
        if (typeof window !== "undefined" && "MercadoPago" in window) {
          // Use type assertion without modifying the global namespace
          const mp = new (window.MercadoPago as any)(process.env.NEXT_PUBLIC_MERCADO_PAGO_PUBLIC_KEY || "")
          setMercadopago(mp)
          setLoading(false)
          return
        }
